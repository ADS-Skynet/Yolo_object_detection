...
YOLO_SKIP       = 2      # run YOLO every N frames (GPU memory constraint)
...
def main(web_port: int = 8082, scenario: int = SCENARIO_LANE_FOLLOW):
    ...
    try:
        ...
        print("[LKAS] Connected")
    except Exception as e:
        print(f"[WARN] LKAS unavailable ({e}) — fixed lane fallback")
    ...
    # ── Image SHM (creator — LKAS detection server reads from here) ──────────
    print("[SHM] Creating image shared memory...")
    ...
    try:
        while running:
            ...
            # Publish to image SHM so LKAS detection server can process it
            image_channel.write(color_bgr, time.time(), frame_id,
            ...
            # ── Save row (rate-limited, always save — every scenario) ─────────
            now = time.monotonic()
            ...
            if web_viewer is not None:
                ...
                web_viewer.broadcast_status({
                    ...
                    'steering':         input_steering,
                    'throttle':         input_throttle,
                    'nearest_distance': min((d for d in distances if d > 0), default=-1.0),
